...
| /MDT           | MDT_meta*         | Hashes for the train and test set used in malware detection tasks. The dataset included in MDT directory is used to evaluate the performance of our dataset in classical machine learning android malware detection task. |
...
There are `readme.md` files in each folder. These readme files will be useful for users to know the detail about the dataset and make use of them quickly. The `APISeqDB `folder include `APISeqDB.pk` and `APISeqDB_load.py`. The first one is the main dataset we release, and the second one is a python format script for readers to quickly start. The `MDT ` folder contains `train.txt` and `test.txt`, they contains hashes which can be used to index api sequence in `APISeqDB.pk ` with our load demo program. Mul directory contains the whole file used in muti-label experiment. `LLM_train ` and `test.json` files are included in the `LLM ` directory. With these two files, we can fine-tunning LLM to qualify the Android malicious behavior description. The following table shows the paths and explanations of the files in our open source dataset directory.
...
| /MDT           | MDT_meta_train.txt | 20,000 hash for the train set                                              |
| /MDT           | MDT_meta_test.txt  | 6,000 hash for the train set                                               |
| /MDT           | readme.md          | detailed user guideline for the usage of MDT_meta* file                    |
...
| ------- | ------------------------- | -------------------------- | ----------------------- |
| $D_c$ | 10k Benign, 10k Malicious | 10k Benign, 10k Malicious  | 3k Benign, 3k Malicious |
| $D_f$ | 10k Benign, 10k Malicious | 115k Benign, 42k Malicious | 3k Benign, 3k Malicious |
